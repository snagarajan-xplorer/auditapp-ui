...
class RegionWiseHeatmapScreen extends StatefulWidget {
  const RegionWiseHeatmapScreen({super.key});

  @override
  State<RegionWiseHeatmapScreen> createState() =>
      _RegionWiseHeatmapScreenState();
}
...
class _RegionWiseHeatmapScreenState extends State<RegionWiseHeatmapScreen> {
  ...
  void _fetchHeatmapData() {
    if (!mounted || selectedZone.isEmpty) return;
    ...
    return ReusableTable(
      columns: [
        TableColumnDef(
          ...
          flex: 2,
          cellBuilder: (row, _) {
            final isTotal = row['_isZoneTotal'] == true;
            return _plainCell(
              isTotal
                  ? '$selectedZone Total'
                  : (row['state'] ?? '-').toString(),
        ...
        TableColumnDef(
          ...
          isLast: true,
          cellBuilder: (row, _) {
            final stateScore = row['state_score'];
            final score = stateScore is Map ? stateScore['score'] : null;
            return _scoreBadgeCell(score);
          },
        ),
      ],
      rows: tableRows,
