...
class NCReportListScreen extends StatefulWidget {
  const NCReportListScreen({super.key});

  @override
  State<NCReportListScreen> createState() => _NCReportListScreenState();
}
...
class _NCReportListScreenState extends State<NCReportListScreen> {
  ...
  Color _colorForRow(dynamic row) {
    final score = row["score"];
    if (score == 0) return const Color(0xFFEA4032);
    if (score == 1) return const Color(0xFFFFB552);
    if (score == 2) return const Color(0xFFFFFD55);
          ...
          children: [
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("NC Report",
                      style: TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.w600,
                          color: Color(0xFF505050))),
                  const SizedBox(height: 18),
                  const Text("Red, Amber & Yellow",
                      style: TextStyle(
                          fontSize: 18,
              ...
              columns: Responsive.isMobile(context)
                  ...
                  : [
                      TableColumnDef(
                        ...
                        cellBuilder: (row, _) => _buildColoredCell(row, "audit_no"),
                      ),
  ...
  Widget _buildColoredCell(dynamic row, String key) {
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 8),
      decoration: BoxDecoration(
        border: Border(left: BorderSide(color: _colorForRow(row), width: 4)),
      ),
      child: Text(
        (row[key] ?? '-').toString(),
